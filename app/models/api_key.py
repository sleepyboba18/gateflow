...
class APIKey(Base):
    ...
    rotation_parent_id: Mapped[uuid.UUID | None] = mapped_column(Uuid(as_uuid=True), ForeignKey("api_keys.id", ondelete="SET NULL"), nullable=True, index=True)
    ...
    ip_rules: Mapped[list["APIKeyIPRule"]] = relationship("APIKeyIPRule", back_populates="api_key", cascade="all, delete-orphan", passive_deletes=True)
    origins: Mapped[list["APIKeyOrigin"]] = relationship("APIKeyOrigin", back_populates="api_key", cascade="all, delete-orphan", passive_deletes=True)
